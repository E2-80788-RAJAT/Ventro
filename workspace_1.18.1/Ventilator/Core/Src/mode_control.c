...
extern int vt_value;
extern int pressure;
extern int PLR_temp_value;
extern int user_flowe;
...
float tidalVolumeIncrement = 0.0;
float duty1 = 0.0f;
...
{
	float K = 0.0000;
	K = (profile == PROFILE_CHILD) ? 0.005f : 0.025f;
...
void Mode_VCV(float targetVolume)
{
	...
	tidalVolumeIncrement = flow * (samplePeriod_ms / 60000.0f); // L
	volumeAccumulator += tidalVolumeIncrement;


	duty1 = ValvePWM_ControlVolume(vt_value, volumeAccumulator);
	ValvePWM_SetDuty(duty1);
...
void Mode_PCV(float targetPressure){
	...
	duty1 = ValvePWM_ControlPressure(PLR_temp_value, Pressure);
	ValvePWM_SetDuty(duty1);
	//		ValvePWM_RampUp(400);
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_15, GPIO_PIN_SET);
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_14, GPIO_PIN_SET);
